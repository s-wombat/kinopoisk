    ...
    <div class="container mx-auto sm:max-w-md mt-6 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg">
        ...
        <form method="POST" action="{{ route('films.store') }}"  enctype="multipart/form-data">
            ...
            <div class="mt-4">
                ...
                <x-textarea id="description" class="block mt-1 w-full" type="text" name="description" :value="old('description')" required autocomplete="description" />
                <x-input-error :messages="$errors->get('description')" class="mt-2" />
            </div>
            <div class="mt-4">
                <x-input-label for="date_release" :value="__('Год выпуска')" />
                <x-text-input id="date_release" class="block mt-1 w-full" type="date" name="date_release" :value="old('date_release')" required autocomplete="date_release" />
